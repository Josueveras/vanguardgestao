...
export const Sidebar: React.FC<SidebarProps> = ({ isCollapsed, setIsCollapsed }) => {
  ...
  return (
    <aside className={`${isCollapsed ? 'w-20' : 'w-64'} bg-vblack text-gray-300 flex flex-col fixed h-full z-20 transition-all duration-300 shadow-xl border-r border-gray-800`}>
      <button
        onClick={() => setIsCollapsed(!isCollapsed)}
        className="absolute -right-3 top-10 bg-vred text-white rounded-full p-1 shadow-lg border-2 border-vblack hover:scale-110 transition-transform z-30"
      >
