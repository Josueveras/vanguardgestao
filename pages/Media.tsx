...
export const MediaModule: React.FC = () => {
  const { content, clients, addContent, updateContent, deleteContent } = useVanguard();
  ...
  const handleDrop = useCallback((e: React.DragEvent, status: ContentItem['status']) => {
      e.preventDefault();
      // Need current value of draggedItem, ideally accessed via ref or state if closure allows. 
      // Simplified here for clarity, assuming re-render provides correct state closure in this setup.
  }, []); 

  // Fix: Drop logic needs access to latest state or use refs. 
  // For safety in this specific architecture, we use a wrapper or pass the ID directly if possible.
  // We'll stick to non-memoized drop handler for correctness unless complex ref logic is added.
  const onDropReal = (e: React.DragEvent, status: ContentItem['status']) => {
       e.preventDefault();
      if (!draggedItem) return;
      
      const item = content.find(c => c.id === draggedItem);
      if (item && item.status !== status) {
          updateContent({ ...item, status });
              ...
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-7xl h-[90vh] flex overflow-hidden animate-fadeIn">
                  ...
                  <div className="w-full lg:w-1/2 flex flex-col border-r border-gray-200">
                      ...
                      <div className="p-6 border-t border-gray-200 bg-gray-50 flex gap-3">
                          {editingItem.id && (
                              <button onClick={() => handleDelete(editingItem.id!)} className="px-5 py-3 border border-red-200 text-red-600 font-bold rounded-lg hover:bg-red-50">Excluir</button>
                          )}
                          <button onClick={handleSave} className="flex-1 bg-vblack text-white font-bold rounded-lg hover:bg-gray-800 transition-colors shadow-lg">
                              Salvar Post
                          </button>
                      </div>
                  </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-1 flex shadow-sm h-10">
            <button 
                ...
                className={`px-3 rounded text-xs font-medium flex items-center gap-1 transition-all ${viewMode === 'board' ? 'bg-gray-100 text-vblack shadow-sm' : 'text-gray-500 hover:text-vblack'}`}
            >
                <SquaresFour size={18} weight={viewMode === 'board' ? 'fill' : 'regular'} />
            </button>
            <button 
                onClick={() => setViewMode('list')} 
                className={`px-3 rounded text-xs font-medium flex items-center gap-1 transition-all ${viewMode === 'list' ? 'bg-gray-100 text-vblack shadow-sm' : 'text-gray-500 hover:text-vblack'}`}
            >
